...
# Capstone Project
# 5/2/23
# Group 6, Skittlz Dynasty, Presentation Group 2, Autoliv Generant Compaction Station Project Group
# HMI for control of Generant Compaction Station
##########################
...
freqBounds1 = [10, 100]                         #Set limitations on allowable frequency [Hz]
freqBounds06 = [10, 125]
freqBounds04 = [10, 150]
freqBoundsCustom = [10, 150]
...
global timeDataRecieved, freqDataRecieved, forceDataRecieved, heightDataRecieved
...
class ResultsTab(customtkinter.CTkTabview):                 #Display results in a tab view within a new window generated in ProcessResults
    def __init__(self, master, **kwargs):
        ...
        self.add("Height Results")

        #Insert results in tabs
        measured_freq = 'Measured frequency:               ' + str(max(freqDataRecieved)) + 'Hz\n\n'
        measured_force = 'Measured force:                        ' + str(max(forceDataRecieved)) + 'N\n\n'
        measured_height = "Measured settling height:       " + str(max(heightDataRecieved)) + "mm\n\n"
        measured_time = "Measured run time:                   " + str(max(timeDataRecieved)) + "sec."
        ...
        self.measuredText.insert("0.0", measured_freq + measured_force + measured_height + measured_time)

        maxFreq = max(freqDataRecieved)
        maxForce = max(forceDataRecieved)
        maxHeight = max(heightDataRecieved)
        ...
        freqPlot.annotate(str(maxFreq), xy=(timeDataRecieved[-1], maxFreq), xytext=(20,0), color='r',       # Annotate the max frequency line
                          textcoords="offset points", va='center')
        freqPlot.plot(timeDataRecieved, freqDataRecieved)                                                       # Plot the measurements
        ...
        forcePlot.annotate(str(maxForce), xy=(timeDataRecieved[-1], maxForce), xytext=(20, 0), color='r',
                          textcoords="offset points", va='center')
        forcePlot.plot(timeDataRecieved, forceDataRecieved)
        ...
        #Rotation plot
        heightFig = Figure(figsize=(5, 5))
        heightPlot = heightFig.add_subplot(111)
        heightPlot.axhline(y=maxHeight, color='r', linestyle='--')
        heightPlot.annotate(str(maxHeight), xy=(timeDataRecieved[-1], maxHeight), xytext=(20, 0), color='r',
                           textcoords="offset points", va='center')
        heightPlot.plot(timeDataRecieved, heightDataRecieved)
        heightPlot.set(xlabel='Time [sec.]', ylabel='Height [mm]')

        heightCanvas = FigureCanvasTkAgg(heightFig, master=self.tab("Height Results"))
        heightCanvas.draw()
        heightCanvas.get_tk_widget().pack(fill='both', padx=10, pady=10)
...
class App(customtkinter.CTk):                               ## Main window with subsytem controls ##
    def __init__(self):
        ...
        self.amp_04 = customtkinter.CTkRadioButton(master=self.radiobutton_frame, text="0.2mm",
        ...
        self.rotateSlider = customtkinter.CTkSlider(self.slide_frame, orientation="horizontal", state="disabled",
                                                    number_of_steps=250, from_=rotSpeedBounds[0], to=rotSpeedBounds[1])
        ...
        self.filenameEntry = customtkinter.CTkEntry(master=self.filename_frame, placeholder_text='File_Name',                   # Create the text entry box
                                                    width=200, height=30) #, validate='focusout',                                  # File name will be checked once the user clicks outside of the entry box
        ...
        self.fileButtonNoSave = customtkinter.CTkRadioButton(master=self.filename_frame, text="Run Without Saving",             # Create radio button for the 'Do Not Save Data' option. Default option
                                                             variable=self.saveData_var, value=1)        # Updates the 'saveFile' function with 'variable' and 'value' to control whether data is saved
        ...
        self.fileButtonSave = customtkinter.CTkRadioButton(master=self.filename_frame, text="Save Data",                        # Create radio button for the 'Do Save Data' option
                                                           variable=self.saveData_var, value=0)          # Updates the 'saveFile' function with 'variable' and 'value' to control whether data is saved
        ...
        ## Call functions to initialize HMI ##
        self.setBounds
    ...
    def setBounds(self):
        ...
        self.freqSlider.set((freqBounds[0]+freqBounds[1])/2)
        self.freqEntry.insert(0, round(self.freqSlider.get(), 1))

        self.forceSlider.set((forceBounds[0]+forceBounds[1])/2)
        self.forceEntry.insert(0, round(self.forceSlider.get(), 1))
        
        self.rotateSlider.set((rotSpeedBounds[0]+rotSpeedBounds[1])/2)
        self.rotateEntry.insert(0, round(self.rotateSlider.get(), 1))

        self.timeSlider.set((timeBounds[0]+timeBounds[1])/2)
        self.timeEntry.insert(0, round(self.timeSlider.get(), 1))

        print('Subsystem bounds for ' + self.amp + ' amplitude')
...
# Function saves the received test data to an excel sheet with the desired filename
    def saveFile(self):
        ...
        if self.saveData_var.get() == 0:                                # If the data is to be saved
            ...
            if fileNameValid:                                           # If the filename is valid
                ...
                wb = Workbook()
                wb.save('C:\\Users\\Vibe Station\\Desktop\\Compaction Station\\Test Data\\'+ str(fileName)+'.xlsx')
                sheet = wb.active
                ...
                sheet['G1'].value = 'Settings'                          # Add appropriate data labels within the excel sheet
                sheet['H1'] = 'Max\nMeasured'
                sheet['I1'] = "Units"
                sheet['H1'].alignment = Alignment(wrapText=True)
                sheet['F2'].value = 'Freq.'
                sheet['F3'].value = 'Force'
                sheet['F4'].value = 'Rot. Freq'
                sheet['F6'].value = 'Time'
                sheet['I2'].value = 'Hz'
                sheet['I3'].value = 'N'
                sheet['I4'].value = 'Hz'
                ...
                sheet['A1'].value = 'Time.'
                sheet['B1'].value = 'Freq'
                sheet['C1'].value = 'Force'
                sheet['D1'].value = 'Height'
                sheet['F5'].value = 'Sett. Hght.'
                sheet['I5'].value = 'mm'
                sheet['G5'].value = 'N/A'
                sheet['F8'].value = 'Cam Amp.'
                sheet['G8'].value = self.amp

                maxFreq = max(freqDataRecieved)
                maxForce = max(forceDataRecieved)
                maxRotation = "N/A"
                maxTime = max(timeDataRecieved)
                maxHeight = max(heightDataRecieved)
                sheet['H5'].value = maxHeight
                #Insert values into sheet
                if self.vibButton.get() == 1:
                    sheet['G2'].value = float(self.freqEntry.get())         # Get the desired subsystems parameters and enter them into the excel sheet
                    sheet['H2'].value = maxFreq
                else:
                    sheet['G2'].value = str("N/A")
                    sheet['H2'].value = str("N/A")

                if self.compactButton.get() == 1:
                    sheet['G3'].value = float(self.forceEntry.get())
                    sheet['H3'].value = maxForce
                else:
                    sheet['G3'].value = str("N/A")
                    sheet['H3'].value = str("N/A")
                
                if self.rotateButton.get() == 1:
                    sheet['G4'].value = float(self.rotateEntry.get())
                    sheet['H4'].value = maxRotation
                else:
                    sheet['G4'].value = str("N/A")
                    sheet['H4'].value = str("N/A")

                if self.timeButton.get() == 1:    
                    sheet['G6'].value = float(self.timeEntry.get())
                    sheet['H6'].value = maxTime
                else:
                    sheet['G6'].value = str("N/A")
                    sheet['H6'].value = maxTime              

                for i in range(len(timeDataRecieved)):
                    sheet.cell(row=2+i, column=1).value = timeDataRecieved[i]

                for i in range(len(freqDataRecieved)):                                  # Enter the entirety of the measured data into the excel sheet
                    sheet.cell(row=2+i, column=2).value = freqDataRecieved[i]

                for i in range(len(forceDataRecieved)):
                    sheet.cell(row=2+i, column=3).value = forceDataRecieved[i]

                for i in range(len(heightDataRecieved)):
                    sheet.cell(row=2+i, column=4).value = heightDataRecieved[i]
                
                wb.save('C:\\Users\\Vibe Station\\Desktop\\Compaction Station\\Test Data\\'+ str(fileName)+'.xlsx')                                      # Save the excel sheet
            else:
                print('Invalid filename')                                           # Return an error if the filename is invalid ********************************************
        elif self.saveData_var.get() == 1:                                                # If the data is not to be saved skip the excel sheet creation
            print("Not Saving Data")

        else:                                                                       # Safety for if something goes wrong
            print('Error with save data RadioButtons')
    ...
    def checkFreqSlider(self, second):                                  # When the Slider is changed, update the Entry box to match
        if isinstance(self.freqEntry.get(), float):                       # Determine if existing entry is an integer
            if self.freqSlider.get() != float(self.freqEntry.get()):      # Determine if existing entry is equal to slider value
...
# All following entry and slider check functions follow the formating of the frequency checks above
    ...
    def checkForceSlider(self, second):                              #When the forceSlider is changed, update the forceEntry box to match
        if isinstance(self.forceEntry.get(), float):
            if self.forceSlider.get() != float(self.forceEntry.get()):
                self.forceEntry.delete(0, len(self.forceEntry.get()))
    ...
    def checkRotateEntry(self):
        ...
        if val1 == True:                                    #If entry is numeric, set slider to entry value
            ...
            self.rotateEntry.delete(0, len(self.rotateEntry.get()))
            print('Invalid oscillation speed')
    ...
    def checkRotateSlider(self, second):                              #When the freqSlider is changed, update the freqEntry box to match
        if isinstance(self.rotateEntry.get(), float):
            if self.rotateSlider.get() != float(self.rotateEntry.get()):
                self.rotateEntry.delete(0, len(self.rotateEntry.get()))
...
# the program receives return data from ClearCore
    def stopButtonFunc(self):
        ...
        clearcore.write(str.encode("STOPNOW:"))                         # Send the stop command to ClearCore. Pointless if test ended due to runTime, used if the 'STOP' button is pressed
...
# Sends desired configuration settings to ClearCore through Serial Communication
    def sendToClearcore(self):
        ...
        # global app
                                                                         # Send the string to ClearCore via Serial Communication
        ...
        for i in range(0, len(splitData)):
            ...
            if text[0] == "Freq":
                ...
                for j in range(0, len(freqDataRecieved)):
                    freqDataRecieved[j] = round(float(freqDataRecieved[j]), 2)
            elif text[0] == "Force":
                forceDataRecieved = text[1].split(",")
                for j in range(0, len(forceDataRecieved)):
                    forceDataRecieved[j] = round(float(forceDataRecieved[j]), 2)
            elif text[0] == "Time":
                timeDataRecieved = text[1].split(",")
                for j in range(0, len(timeDataRecieved)):
                    timeDataRecieved[j] = float(timeDataRecieved[j])
            elif text[0] == "Height":
                heightDataRecieved = text[1].split(",")
                for j in range(0, len(heightDataRecieved)):
                    heightDataRecieved[j] = round(float(heightDataRecieved[j]), 2)
            elif text[0] == "STOP":
                self.finishTest()
            else:
                print(text[0])
        
        #return timeDataRecieved, freqDataRecieved, forceDataRecieved, heightDataRecieved

    def checkSerial(self):
        if clearcore.inWaiting() > 0:
                self.readData()
        app.after(500, self.checkSerial)
